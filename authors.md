# Authors and Citation

## Authors

- **Cristián Ayala**. Author, maintainer.

- **Cristina Marchant**. Contributor.

- **Jonatan Salazar**. Contributor.

- **[DESUC](https://github.com/desuc)**. Funder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/desuc/desuctools/blob/master/DESCRIPTION)

Ayala C (2026). *desuctools: Funciones para trabajo en DESUC*. R package
version 0.2.0.9005, <https://desuc.github.io/desuctools>.

    @Manual{,
      title = {desuctools: Funciones para trabajo en DESUC},
      author = {Cristián Ayala},
      year = {2026},
      note = {R package version 0.2.0.9005},
      url = {https://desuc.github.io/desuctools},
    }
